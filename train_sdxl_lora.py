...
if train_mode.startswith("sdxl"):
    laungh_script = f"sdxl_{laungh_script}"
    if min_timestep != 0:
...
if output_config:
    ...
    ext_args.append("--config_file={}".format(config_file))
...
if enable_block_weights:
    ...
    if dropout:
        ext_args.append("dropout=" + dropout)
    ext_args.append("algo=" + algo)
elif enable_dylora:
    ext_args.append("--network_module=networks.dylora")
    ext_args.append("--network_args")
    ext_args.append("unit=" + unit)
    if conv_dim:
        ext_args.append("conv_dim=" + conv_dim)
        if conv_alpha:
            ext_args.append("conv_alpha=" + conv_alpha)
    if module_dropout:
        ext_args.append("module_dropout=" + module_dropout)
else:
    if conv_dim:
        ext_args.append("--network_args")
        ext_args.append("conv_dim=" + conv_dim)
...
ext_args_command = " ".join(ext_args)
